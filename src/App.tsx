          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <button 
                ...
                className={cn(
                  "flex items-center gap-2 px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all border shadow-sm",
                  filtroStatus !== 'TODOS' ? "bg-giro-primary text-white border-giro-primary shadow-giro-primary/20" : "bg-white text-giro-text border-gray-100 hover:border-gray-200"
                )}
              >
                ...
                {isFilterOpen && (
                  <>
                    ...
                    <motion.div
                      ...
                      className="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-2xl border border-gray-100 z-[101] overflow-hidden py-1.5 p-1"
                    >
                      {(['TODOS', 'ATRASO', 'DIA', 'QUITADO'] as const).map(f => (
                        <button
                          key={f}
                          onClick={() => { setFiltroStatus(f); setIsFilterOpen(false); }}
                          className={cn(
                            "w-full text-left px-4 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-colors flex items-center justify-between",
                            filtroStatus === f ? "bg-giro-primary text-white" : "text-giro-text-muted hover:bg-gray-50"
                          )}
                        >
                          {f === 'TODOS' ? 'Todos' : f === 'ATRASO' ? 'Atrasados' : f === 'DIA' ? 'Em Dia' : 'Quitados'}
                          {filtroStatus === f && <Check className="w-3.5 h-3.5 text-white" />}
                        </button>
                      ))}
                    </motion.div>
                  </>
